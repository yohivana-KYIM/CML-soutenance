    ...
    <form wire:submit.prevent="mount">
        ...
        <div class="mb-4">
            <input  type="text"
                ...
                wire:model="state.description" id="description" placeholder="description" name="description" aria-label="description" aria-describedby="email-addon"
                required autofocus />
            @error('description')
...
<div class="mb-4">
            <input  type="text"
                class="text-size-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow"
                wire:model="state.libelle" id="libelle"   placeholder="libelle" name="libelle" aria-label="fonction" aria-describedby="email-addon"
                required autofocus />
            @error('libelle')
            <p class="text-size-sm text-red-500">{{ $message }}</p>
            @enderror
